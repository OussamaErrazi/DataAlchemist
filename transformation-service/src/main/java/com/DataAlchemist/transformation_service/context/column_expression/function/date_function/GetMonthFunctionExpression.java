...
public class GetMonthFunctionExpression implements ColumnExpression {
    private final ColumnExpression input;

    public GetMonthFunctionExpression(ColumnExpression input) {
        this.input = input;
    }

    @Override
    public Cell evaluate(Row row) {
        if(input == null) throw new IllegalArgumentException("Unexpected input, expected date input value in get_month function but found null");
        Cell inputDate = input.evaluate(row);
        if(inputDate.getColumnType() != ColumnType.DATE) throw new IllegalArgumentException("Expected a date in get_month function 1st arg but got "+inputDate.getColumnType());
        String datePattern = "^date\\(([0-9]{0,2}),([0-9]{0,2}),([0-9]{0,4})\\)$";
        Pattern pattern = Pattern.compile(datePattern);
        Matcher matcher = pattern.matcher(inputDate.getValue().toString());
        if(matcher.matches()) {
            Integer month = matcher.group(2).isEmpty() ? null : Integer.parseInt(matcher.group(2));
            return Cell.builder()
                    .value(month)
                    .columnType(ColumnType.INTEGER)
                    .columnName("get_month("+inputDate.getColumnName()+")")
                    .build();
        }
        try{
            ...
            throw new IllegalArgumentException("Invalid date format "+inputDate.getValue().toString()+" in get_month function");
        }
    }
}
