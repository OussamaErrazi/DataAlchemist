...
public class MonthsBetweenFunctionExpression implements ColumnExpression {
    private final ColumnExpression date1;
    private final ColumnExpression date2;

    public MonthsBetweenFunctionExpression(ColumnExpression date1, ColumnExpression date2) {
        this.date1 = date1;
        this.date2 = date2;
    }
    @Override
    public Cell evaluate(Row row) {
        if(date1 == null || date2 == null) throw new IllegalArgumentException("Unexpected input, expected date input value in months_between function but found null");
        Cell date1Value = date1.evaluate(row);
        if(date1Value.getColumnType() != ColumnType.DATE) throw new IllegalArgumentException("Expected a date in months_between function 1st arg but got "+date1Value.getColumnType());

        Cell date2Value = date2.evaluate(row);
        if(date2Value.getColumnType() != ColumnType.DATE) throw new IllegalArgumentException("Expected a date in months_between function 2nd arg but got "+date2Value.getColumnType());
        if(date1Value.getValue() == null || date2Value.getValue() == null)
            return Cell.builder()
                    .value(null)
                    .columnType(ColumnType.INTEGER)
                    .columnName("months_between("+date1Value.getColumnName()+","+date2Value.getColumnName()+")")
                    .build();
        ...
        return Cell.builder()
                .value(ChronoUnit.MONTHS.between(LocalDate.of(year1, month1, day1), LocalDate.of(year2, month2, day2)))
                .columnType(ColumnType.INTEGER)
                .columnName("months_between("+date1Value.getColumnName()+","+date2Value.getColumnName()+")")
                .build();
    }
