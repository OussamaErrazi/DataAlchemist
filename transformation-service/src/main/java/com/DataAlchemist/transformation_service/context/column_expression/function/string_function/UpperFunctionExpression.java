...
public class UpperFunctionExpression implements ColumnExpression {
    private final ColumnExpression inputString;

    public UpperFunctionExpression(ColumnExpression inputString) {
    ...
    public Cell evaluate(Row row) {
        ...
        if(result.getColumnType() != ColumnType.STRING) throw new IllegalArgumentException("Expected a string in upper function but got "+result.getColumnType());
        if(result.getValue() == null) return result;
        result.setValue(result.getValue().toString().toUpperCase());
        return result;
    }
}
